<template>
  <el-dialog v-model="dialogVisible" title="可见老师" width="45%">
...
if (props.lista.length) {
  Classes.value = props.lista.map((item:any) => {
    return item.id;
  })
}
const form = reactive({
...
let add = async () => {
  ...
  console.log(13124,res);
  if (res.errCode === 10000) {
    props.fla()
    emit('all', Classes.value.map((item: any) => ({ ...item, id: item })))
    Classes.value = ""
  }
}
</script>
